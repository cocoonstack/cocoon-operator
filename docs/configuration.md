...
| `GOOGLE_APPLICATION_CREDENTIALS` | unset | Path to a GCP service-account key file for the ADC leg of the registry keychain; set by the `sa-key` overlay's deployment patch when running off a mounted key instead of workload identity. |
